...
int ec_readstate(void)
{
    ...
    {
        configadr = ec_slave[slave].configadr;
        rval = etohs( ec_FPRDw(configadr, ECT_REG_ALSTAT, EC_TIMEOUTRET) );
        ec_slave[slave].ALstatuscode = etohs( ec_FPRDw(configadr, ECT_REG_ALSTATCODE, EC_TIMEOUTRET) );
		if (rval < lowest)
			lowest = rval;
...
{
    uint16 loop, start, end, configadr;
    ...
    {
        configadr = ec_slave[loop].configadr;
...
int ec_eeprom2master(uint16 slave)
{
	...
	if ( ec_slave[slave].eep_pdi )
	{
    	configadr = ec_slave[slave].configadr; 
		eepctl = 2;
		wck=ec_FPWR(configadr, ECT_REG_EEPCFG, sizeof(eepctl), &eepctl , EC_TIMEOUTRET); /* force Eeprom from PDI */
		eepctl = 0;
		wck=ec_FPWR(configadr, ECT_REG_EEPCFG, sizeof(eepctl), &eepctl , EC_TIMEOUTRET); /* set Eeprom to master */
...
int ec_eeprom2pdi(uint16 slave)
{
	...
	if ( !ec_slave[slave].eep_pdi )
	{
    	configadr = ec_slave[slave].configadr; 
		eepctl = 1;
		wck=ec_FPWR(configadr, ECT_REG_EEPCFG, sizeof(eepctl), &eepctl , EC_TIMEOUTRET); /* set Eeprom to PDI */
...
uint64 ec_readeepromFP(uint16 configadr, uint16 eeproma, int timeout)
{
	...
	if (wkc)
	{
		...
		while (((estat & EC_ESTAT_BUSY) > 0) && (timercmp(&tv2, &tve, <))); /* wait for eeprom ready */
...
void ec_readeeprom1(uint16 slave, uint16 eeproma)
{
    ...
    ed.addr = htoes(eeproma);
    ed.d2   = 0x0000;
    ec_FPWR(configadr, ECT_REG_EEPCTL, sizeof(ed), &ed, EC_TIMEOUTRET);
}
...
int ec_send_processdata(void)
{
	uint32 LogAdr;
	uint16 w1, w2;
	int length, sublength, maxlength;
    uint8 idx;
	int wkc;
	void* data;
	boolean first=FALSE;
	...
	length = ec_slave[0].Obytes + ec_slave[0].Ibytes;
	LogAdr = 0;
	if (length)
	{	
		...
		if (ec_slave[0].blockLRW)
		{
			/* if inputs available generate LRD */
			if(ec_slave[0].Ibytes)
			{
				data=ec_slave[0].inputs;
				length = ec_slave[0].Ibytes;
				LogAdr = 0 + ec_slave[0].Obytes;
				/* segment transfer if needed */
				do
				{			
					sublength = length;
					if(sublength > EC_MAXLRWDATA)
					{
						sublength = EC_MAXLRWDATA;
					}	
					/* get new index */
				    idx = ec_getindex();
					w1 = LO_WORD(LogAdr);
			...
			if(ec_slave[0].Obytes)
			{
				data=ec_slave[0].outputs;
				length = ec_slave[0].Obytes;
				LogAdr = 0;
				/* segment transfer if needed */
				do
				{			
			...
			{			
				...
				if(first)
				{
				    ...
				    ec_DCtO = ec_adddatagram(&ec_txbuf[idx], EC_CMD_FRMW, idx, FALSE, 
											 ec_slave[ec_slave[0].DCnext].configadr, 
