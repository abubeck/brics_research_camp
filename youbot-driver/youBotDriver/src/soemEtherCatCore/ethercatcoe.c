...
int ec_readPDOmap(uint16 Slave, int *Osize, int *Isize)
{
	...
	*Osize = 0;
	...
	/* read rxPDO assign subindex 0 ( = number of rxPDO's) */
    wkc = ec_SDOread(Slave, ECT_SDO_RXPDOASSIGN, 0x00, FALSE, &rdl, &rdat, EC_TIMEOUTRXM);
	rdat = etohs(rdat);
    ...
    if ((wkc > 0) && (rdat > 0))
    {
        ...
        for (idxloop = 1; idxloop <= nidx; idxloop++)
        {
            rdl = sizeof(rdat); rdat = 0;
			/* read rxPDO assign */
            wkc = ec_SDOread(Slave, ECT_SDO_RXPDOASSIGN, (uint8)idxloop, FALSE, &rdl, &rdat, EC_TIMEOUTRXM);
			/* result is index of rxPDO */
            idx = etohl(rdat);
            if (idx > 0)
            {
                rdl = sizeof(subcnt); subcnt = 0;
				/* read number of subindexes of rxPDO */
                ...
                for (subidxloop = 1; subidxloop <= subidx; subidxloop++)
                {
					...
					/* read SDO that is mapped in rxPDO */
		...
		/* return total found output bitlength (rxPDO) */
        *Osize = bsize;
    };

    rdl = sizeof(rdat); rdat = 0;
	/* read txPDO assign subindex 0 ( = number of txPDO's) */
    wkc = ec_SDOread(Slave, ECT_SDO_TXPDOASSIGN, 0x00, FALSE, &rdl, &rdat, EC_TIMEOUTRXM);
	rdat = etohs(rdat);
	/* positive result from slave ? */
    if ((wkc > 0) && (rdat > 0))
    {
        ...
        for (idxloop = 1; idxloop <= nidx; idxloop++)
        {
            rdl = sizeof(rdat2); rdat2 = 0;
			/* read txPDO assign */
            wkc = ec_SDOread(Slave, ECT_SDO_TXPDOASSIGN, (uint8)idxloop, FALSE, &rdl, &rdat2, EC_TIMEOUTRXM);
			rdat2 = etohl(rdat2);
			/* result is index of txPDO */
            idx = rdat2;
            if (idx > 0)
            {
                rdl = sizeof(rdat2); rdat2 = 0;
				/* read number of subindexes of txPDO */
                wkc = ec_SDOread(Slave,idx, 0x00, FALSE, &rdl, &rdat2, EC_TIMEOUTRXM);
                subidx = (uint16)etohl(rdat2);
				/* for each subindex */
                for (subidxloop = 1; subidxloop <= subidx; subidxloop++)
                {
                    rdl = sizeof(rdat2); rdat2 = 0;
					/* read SDO that is mapped in txPDO */
                    wkc = ec_SDOread(Slave, idx, (uint8)subidxloop, FALSE, &rdl, &rdat2, EC_TIMEOUTRXM);
					rdat2 = etohl(rdat2);
					/* extract bitlength of SDO */
                    bsize += LO_BYTE(rdat2);
		...
		/* return total found input bitlength (txPDO) */
        *Isize = bsize;
    };
...
int ec_readPDOmapCA(uint16 Slave, int *Osize, int *Isize)
{
	...
	*Osize = 0;
    ...
    wkc = ec_SDOread(Slave, ECT_SDO_RXPDOASSIGN, 0x00, TRUE, &rdl, &ec_PDOassign, EC_TIMEOUTRXM);
	/* positive result from slave ? */
    if ((wkc > 0) && (ec_PDOassign.n > 0))
    {
        ...
        for (idxloop = 1; idxloop <= nidx; idxloop++)
        {
            ...
            if (idx > 0)
            {
				...
				/* read SDO's that are mapped in rxPDO, CA mode */
    ...
    rdl = sizeof(ec_PDOassign); ec_PDOassign.n=0;
	/* read txPDOassign in CA mode, all subindexes are read in one struct */
    wkc = ec_SDOread(Slave, ECT_SDO_TXPDOASSIGN, 0x00, TRUE, &rdl, &ec_PDOassign, EC_TIMEOUTRXM);
	/* positive result from slave ? */
    if ((wkc > 0) && (ec_PDOassign.n > 0))
    {
        nidx = ec_PDOassign.n;
        ...
        for (idxloop = 1; idxloop <= nidx; idxloop++)
        {
            ...
            {
                rdl = sizeof(ec_PDOdesc); ec_PDOdesc.n = 0;
				/* read SDO's that are mapped in txPDO, CA mode */
                wkc = ec_SDOread(Slave,idx, 0x00, TRUE, &rdl, &ec_PDOdesc, EC_TIMEOUTRXM);
                subidx = ec_PDOdesc.n;
				/* extract all bitlengths of SDO's */
                for (subidxloop = 1; subidxloop <= subidx; subidxloop++)
